...
export default function BookmarkedExhibitionPopup({ isOpen, onClose, user, alarmExhibition }) {
  ...
  const fetchBookmarkedExhibitions = async () => {
    try {
      ...
      const exhibitionsData = await Promise.all(
        bookmarkData
          .filter(bookmark => bookmark.exhibition_id)
          .map(async (bookmark) => {
      ...
      const galleriesData = await Promise.all(
        bookmarkData
          .filter(bookmark => bookmark.gallery_id)
          .map(async (bookmark) => {
      ...
      const productsData = await Promise.all(
        bookmarkData
          .filter(bookmark => bookmark.product_id)
          .map(async (bookmark) => {
        ...
        >
          ...
          <div className="flex items-center justify-between sticky top-0 bg-white/95 backdrop-blur-md z-10 border-b border-gray-200 p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900">즐겨찾기</h2>
                <p className="text-sm text-gray-600">저장한 전시회, 갤러리, 작품</p>
              </div>
            </div>
            
            <div className="text-right">
              <div className="text-2xl font-bold text-gray-900">{finalExhibitions.length}</div>
              <div className="text-xs text-gray-500">저장한 항목</div>
            </div>
          </div>
